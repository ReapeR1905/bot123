...
module.exports.run = async (client, msg, args) => {
  ...
  const adgir = new Discord.RichEmbed()
.setColor("#c3c3c3")
.setDescription(`:pencil: **Lütfen Yazmak İstediğiniz** **\`Oyun İsmini\`** **Yazın.**\n\n:gear: **__Doğru Kullanım:__**\n\n**\`${prefix}oyunara <oyunismi>\`**`)
  if (!query) return msg.channel.send(adgir)
  ...
  if (query === "minecraft" || query === "mc" || query === "Minecraft" || query === "MC" || query === "Mc" || query === "MİNECRAFT" || query === "MINECRAFT" ) {
    ...
    const embed = new Discord.RichEmbed()
      ...
      .setColor("#c3c3c3")
      .setDescription(`<a:anlasilmadi:705508503678091406> **Bir Hata Oluştu! \n**Hata:** \n**\`${err.message}\`**`)
			return msg.channel.send(hata);
    }
    return;
  }
  ...
  try {
    ...
    const oyunyok = new Discord.RichEmbed()
    .setColor("#c3c3c3")
    .setDescription(`:pencil: **Yazdığınız İsimde Bir** **\`Oyun\`** **Bulunamadı.**\n\n:gear: **__Doğru Kullanım:__**\n\n**\`${prefix}oyunara <oyunismi>\`**`)
			if (!id) return msg.channel.send('Bulunamadı');
